% Simple script to save as singel time steps (slice) of a SnowModel .gdat 
% data as .txt files for a specified varable of intrest.
%
...
slice_timestep = [175]; % April 1, 2015, This is a vector of all the time steps you are intrested in 

[X,Y,Z,timevec,xmax,ymax] = readGDAT(startdate,timesteps,ctl_file,var_name,xll,yll,nx,ny,cell,slice_timestep);
...
for i = 1:length(slice_timestep)
file_name = strcat(folder_name,'/',var_name,'_slice_',num2str(slice_timestep(i)),'.txt')
Time_Slice = Z(:,:,i);
...
function [X,Y,Z,timevec,xmax,ymax] = readGDAT(startdate,timesteps,ctl_file,var_name,xll,yll,nx,ny,cell,slice_timestep,time_offset)
    ...
    timevec=timevec(1,slice_timestep)
    xmin=xll;
    ymin=yll;
    ...
    for j=1:length(timevec); % Loop to create the Z cube of only the specific time steps
